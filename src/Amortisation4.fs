...
module Amortisation4 =
    ...
    module Streams =
        ...
        let map (f: 'T -> 'U) (stream: Stream<'T>) : Stream<'U> = {
            Subscribe = fun observer -> stream.Subscribe(f >> observer)
        }
        ...
        let combine (f: 'T -> 'U -> 'V) (stream1: Stream<'T>) (stream2: Stream<'U>) : Stream<'V> = {
            Subscribe =
                fun observer ->
                    ...
                    let disp1 =
                        stream1.Subscribe(fun x ->
                            latest1 := Some x
                            checkEmit ()
                        )

                    let disp2 =
                        stream2.Subscribe(fun x ->
                            latest2 := Some x
                            checkEmit ()
                        )
        ...
        let toList (stream: Stream<'T>) : 'T list =
            ...
            use subscription = stream.Subscribe(fun x -> results.Add(x))
    ...
    let processPaymentStream
        (parameters: Parameters)
        (initialState: AmortisationState)
        : Streams.Stream<AmortisationEvent> -> Streams.Stream<AmortisationState> =
        fun eventStream ->
            eventStream
            |> Streams.scan
                (fun state event ->
                    ...
                    | BalanceUpdated newBalance -> { state with TotalBalance = newBalance }
                )
                initialState

    /// Generate interest accrual events stream
    let generateInterestStream
        (parameters: Parameters)
        (balanceStream: Streams.Stream<AmortisationState>)
        : Streams.Stream<AmortisationEvent> =
        balanceStream
    ...
    let generateFeeStream
        (parameters: Parameters)
        (dayStream: Streams.Stream<int<OffsetDay>>)
        : Streams.Stream<AmortisationEvent> =
        dayStream
        |> Streams.filter (fun day ->
            parameters.FeeTypes
            |> List.exists (fun feeType -> Fee.isApplicableOnDay feeType day)
        )
        |> Streams.map (fun day ->
    ...
    let generatePaymentStream (scheduledPayments: ScheduledPayment array) : Streams.Stream<AmortisationEvent> =
        scheduledPayments |> Streams.fromSeq |> Streams.map PaymentDue
    ...
    let calculateBasicScheduleStream (parameters: Parameters) : Streams.Stream<int<OffsetDay>> =
        ...
        [ 1<OffsetDay> .. sp.FinalPaymentDay ] |> Streams.fromSeq
    ...
    let amortiseReactive (parameters: Parameters) (scheduledPayments: ScheduledPayment array) : ScheduleItem array =
        ...
        let interestEventStream =
            generateInterestStream parameters (Streams.fromSeq [ initialState ])
        ...
        let combinedEventStream =
            ...
            |> List.fold (fun acc stream -> Streams.combine (fun _ event -> event) acc stream) paymentEventStream
    ...
    let amortiseWithStreams (parameters: Parameters) (scheduledPayments: ScheduledPayment array) : ScheduleItem array =
        ...
        let balanceStream =
            dayStream
            |> Streams.scan
                (fun (prevBalance, prevInterest, prevFees) currentDay ->
                    // Find any scheduled payment for this day
                    let paymentForDay =
                        scheduledPayments |> Array.tryFind (fun sp -> sp.OffsetDay = currentDay)
                    ...
                    | None -> (prevBalance, newInterest, newFees)
                )
        ...
        let scheduleItemStream =
            Streams.combine
                (fun day (principal, interest, fees) -> ScheduleItem.create day principal interest fees)
                dayStream
                balanceStream

        // Collect results
        Streams.toList scheduleItemStream |> Array.ofList
    ...
    let amortise (parameters: Parameters) (scheduledPayments: ScheduledPayment array) : ScheduleItem array =
        ...
        let paymentMap =
            scheduledPayments |> Array.map (fun sp -> sp.OffsetDay, sp) |> Map.ofArray
        ...
        let finalScheduleItems =
            scheduleStream
            |> Streams.scan
                (fun (scheduleMap, currentPrincipal, currentInterest, currentFees) offsetDay ->
                    ...
                    let (finalPrincipal, finalInterest, finalFees) =
                        ...
                        | None -> (currentPrincipal, newInterest, newFees)

                    // Create schedule item
                    let scheduleItem =
                        ScheduleItem.create offsetDay finalPrincipal finalInterest finalFees

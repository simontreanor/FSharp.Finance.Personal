...
module Amortisation =
    ...
    type ScheduleItem = {
        ...
        GeneratedPayment: GeneratedPayment voption
    ...
    let calculate sp (earlySettlementDate: Date voption) originalFinalPaymentDay applyNegativeInterest appliedPayments =
        ...
        let appliedPayments' =
            if Array.isEmpty appliedPayments || (Array.head appliedPayments |> fun ap -> ap.AppliedPaymentDay <> 0<OffsetDay>) then // deals both with no payments and manual payment schedules
                let dummyPayment: AppliedPayment = { AppliedPaymentDay = 0<OffsetDay>; ScheduledPayment = 0L<Cent>; ActualPayments = [||]; GeneratedPayment = ValueNone; IncurredCharges = [||]; NetEffect = 0L<Cent>; PaymentStatus = ValueNone }
        ...
        |> Array.scan(fun (a: ScheduleItem) ap ->
...

            {
                ...
                PaymentStatus = if a.BalanceStatus = Settlement then ValueNone elif isExtraPayment then ValueSome ExtraPayment elif isOverpayment then ValueSome Overpayment else ap.PaymentStatus

...
module Amortisation =
    ...
    module OffsetDayType =
        ...
        let toHtml (offsetDay: uint<OffsetDay>) offsetDayType =
    ...
    type ScheduleItem = {
        ...
        NewInterest: Cent.PrecisionCent
    ...
    module ScheduleItem =
        ...
        let private formatActualPayments =
            Map.map (fun (index: uint) actualPayment -> $"<b>{index}</b>&nbsp;{actualPayment}")
            >> Map.values
            >> String.concat "<br/>"
            >> _.Replace(" ", "&nbsp;")

        let private formatPaidBy =
            Map.map (fun (day: uint<OffsetDay>, index: uint) amount -> $"<b>{day}#{index}</b>&nbsp;{amount:N2}")
            >> Map.values
            >> String.concat "<br/>"
            >> _.Replace(" ", "&nbsp;")
        ...
        let toHtmlRow (p: Parameters) settlementDay offsetDay scheduleItem =
            let fields =
                [|
                    ...
                    yield "", $"""{scheduleItem.Advances |> Array.map (fun t -> $"{t:N2}") |> Array.toStringOrNa}"""
                    yield " style=\"white-space: nowrap;\"", $"{scheduleItem.ScheduledPayment}"
                    yield "", $"{scheduleItem.Window}"
                    yield "", $"{scheduleItem.PaymentDue:N2}"
                    ...
                    yield "", $"{scheduleItem.NetEffect:N2}"
                    ...
                    if p.Advanced.ChargeConfig.IsSome then
                        yield "", $"{scheduleItem.ChargesPortion:N2}"
                    yield "", $"{formatDecimalCent scheduleItem.ActuarialInterest}"
                    yield "", $"{formatDecimalCent scheduleItem.NewInterest}"
                    yield "", $"{scheduleItem.InterestPortion:N2}"
                    if p.Basic.FeeConfig.IsSome then
                        yield "", $"{scheduleItem.FeeRebateIfSettled:N2}"
                    if p.Basic.FeeConfig.IsSome then
                        yield "", $"{scheduleItem.FeeRebate:N2}"
                    if p.Basic.FeeConfig.IsSome then
                        yield "", $"{scheduleItem.FeePortion:N2}"
                    yield "", $"{scheduleItem.PrincipalPortion:N2}"
                    if p.Advanced.ChargeConfig.IsSome then
                        yield "", $"{scheduleItem.ChargesBalance:N2}"
                    yield "", $"{formatDecimalCent scheduleItem.InterestBalance}"
                    if p.Basic.FeeConfig.IsSome then
                        yield "", $"{scheduleItem.FeeBalance:N2}"
                    yield "", $"{scheduleItem.PrincipalBalance:N2}"
                    yield "", $"{scheduleItem.SettlementFigure:N2}"
                |]
    ...
    type FinalStats = {
        ...
        SettlementFigure: (uint<OffsetDay> * Cent.Signed) voption
...
"""
    ...
    type Schedule = {
        ...
        ScheduleItems: Map<uint<OffsetDay>, ScheduleItem>
...
<h4>Final Stats</h4>{FinalStats.toHtmlTable generationResult.AmortisationSchedule.FinalStats}"""
    ...
    let getWindow unitPeriodMap (currentDay: uint<OffsetDay>) currentScheduledPayment previousWindow =
    ...
    let getAdvances currentDay (principal: Cent.Unsigned) =
        if currentDay = 0u<OffsetDay> then [| principal |] else [||]
    ...
    let feePercentage (principal: Cent.Unsigned) feeTotal =
        if principal = 0uL<Cent> then
            Percent 0m
        else
    ...
    let isSettledWithinGracePeriod (p: Parameters) =
        ...
        | SettlementDay.SettlementOnEvaluationDay ->
            ...
            <= p.Advanced.InterestConfig.InitialGracePeriod
    ...
    let calculateFee feeTotal appliedPaymentDay originalFinalPaymentDay : Cent.Unsigned =
        if originalFinalPaymentDay <= 0u<OffsetDay> then
            0uL<Cent>
    ...
    let calculatePaymentDue
        si
        (originalPayment: Cent.Unsigned voption)
        rescheduledPayment
        (extraPaymentsBalance: Cent.Signed)
        (interestPortionL: Cent.Signed)
        minimumPayment
        : Cent.Unsigned =
        ...
        // otherwise, calculate the payment due based on scheduled payments and various balances
            ...
            | _, ValueSome rp -> Cent.transferToPortion rp.Value
            // if the original payment is cancelled due to rescheduling, there nothing is due
            | ValueSome op, _ when op = 0uL<Cent> -> 0L<Cent>
            // reduce the payment due if early/extra payments have been made
            | ValueSome op, _ when extraPaymentsBalance > 0L<Cent> -> Cent.transferToPortion op - extraPaymentsBalance
            // non-zero original payments are due in full
            | ValueSome op, _ -> Cent.transferToPortion op
            ...
            |> fun payment ->
                ...
                | DeferOrWriteOff minimumPayment when payment < minimumPayment -> 0uL<Cent>
    ...
    let calculateStatutoryFeeRebate
        bp
        (appliedPayments: Map<uint<OffsetDay>, AppliedPayment>)
        initialStats
        appliedPaymentDay
        window
        =
        ...
        let previousScheduledPaymentDay =
            originalScheduledPayments
            |> Array.filter (fun osp -> osp.OffsetDay <= appliedPaymentDay)
            |> Array.last
            |> _.OffsetDay

        let numerator = appliedPaymentDay - previousScheduledPaymentDay |> uint
    ...
    let calculateActuarialInterest
        ...
        (previousDay: uint<OffsetDay>)
        (currentDay: uint<OffsetDay>)
    ...
    let calculateInterestAdjustment
        ...
        | Interest.Method.AddOn when
            ...
            |> Interest.Cap.cappedAddedValue
                ...
                (Cent.transferToPortion basicParameters.Principal)
                cumulativeActuarialInterestM
        | _ -> 0m<Cent>
    ...
    /// apportions the fee
    let apportionFee
        (basicFeeConfig: Fee.BasicConfig voption)
        previousFeeBalance
        assignable
        principal
        feeTotal
    ...
    let calculateFeeRebate
        ...
        | ValueSome feeConfig ->
            ...
            | Fee.SettlementRebate.ProRataRescheduled originalFinalPaymentDay ->
                calculateFee feeTotal currentDay originalFinalPaymentDay
            | Fee.SettlementRebate.FeeBalance -> cumulativeFee
            | Fee.SettlementRebate.Zero -> 0uL<Cent>
        | ValueNone -> 0uL<Cent>
    ...
    let createInitialScheduleItem startDate principal fee interest (advancedFeeConfig: Fee.AdvancedConfig voption) = {
        ScheduleItem.zero with
            ...
            Advances = [| principal |]
            PrincipalBalance = Cent.transferToPortion principal
            FeeBalance = fee
            InterestBalance = interest
            SettlementFigure = Cent.transferToPortion principal + fee
            FeeRebateIfSettled =
                match advancedFeeConfig with
                | ValueSome fc ->
                    match fc.SettlementRebate with
                    | Fee.SettlementRebate.Zero -> 0uL<Cent>
                    | _ -> Cent.portionToTransfer fee
                | ValueNone -> 0uL<Cent>
    }

    let createInitialTotals interest = {
        CumulativeScheduledPayments = 0uL<Cent>
        CumulativeActualPayments = 0L<Cent>
        CumulativeGeneratedPayments = 0L<Cent>
        CumulativeFee = 0uL<Cent>
        CumulativeInterest = interest
        CumulativeInterestPortions = 0uL<Cent>
        CumulativeActuarialInterestM = 0m<Cent>
    }

    // removes duplicated initial offset day
    let deduplicateDay0 (a: (uint<OffsetDay> * ScheduleItem) array) =
        if a |> Array.filter (fun (day, _) -> day = 0u<OffsetDay>) |> Array.length = 2 then
            a |> Array.tail
        else
            a
        ...
        | AutoGenerateSchedule ags ->
            let paymentSchedule =
                UnitPeriod.generatePaymentSchedule
                    (UnitPeriod.ScheduleLength.MaxDuration(startDate, maxAppliedPaymentDay))
            ...
            let dayToUnitPeriodMap =
                [| 0u .. uint maxAppliedPaymentDay |]
                |> Array.map (fun day ->
                    let day = day * 1u<OffsetDay>
    ...
    let internal calculate (p: Parameters) initialStats (appliedPayments: Map<uint<OffsetDay>, AppliedPayment>) =
        ...
        let generator ((previousDay, previous), totals) (currentDay, current: AppliedPayment) =
            ...
            let finalInterestReductionL =
                finalInterestReductionM |> Cent.precisionToPortion interestRounding
            ...
            let interestPortionL = interestPortionM |> Cent.precisionToPortion interestRounding
            ...
            let extraPaymentsBalance =
                ...
                - Cent.transferToPortion totals.CumulativeScheduledPayments
            ...
            let (newChargesTotal: Cent.Unsigned), incurredCharges =
                if paymentDue = 0uL<Cent> then
                    0uL<Cent>, [||]
                else
                    current.AppliedCharges |> Array.sumBy _.Total, current.AppliedCharges

            let newChargesTotal' = newChargesTotal + previous.ChargesBalance
            ...
            let netEffect =
                if currentDay > evaluationDay then
                    min current.NetEffect (Cent.transferToPortion paymentDue)
                else
                    current.NetEffect
            ...
            let cumulativeActuarialInterestL =
                totals'.CumulativeActuarialInterestM |> Cent.precisionToPortion interestRounding

            let settlement, principalForgiven =
                Cent.transferToPortion p.Basic.Principal + cumulativeActuarialInterestL
                - totals'.CumulativeActualPayments
                |> fun s ->
            ...
            let interestAdjustmentL =
                interestAdjustmentM |> Cent.precisionToPortion interestRounding
            ...
            let interestPortionL' =
                ...
                |> Cent.precisionToPortion interestRounding
            ...
            let assignable, scheduledPaymentAdjustment =
                if netEffect = 0L<Cent> then
                    0L<Cent>, 0L<Cent>
                elif netEffect < 0L<Cent> then
                    -netEffect + interestPortionL', 0L<Cent>
                else
                    match p.Advanced.PaymentConfig.ScheduledPaymentOption with
                    | AsScheduled -> netEffect - Cent.transferToPortion newChargesTotal' - interestPortionL', 0L<Cent>
                    | AddChargesAndInterest -> netEffect, Cent.transferToPortion newChargesTotal' - interestPortionL'
            ...
            let generatedSettlementPayment' =
                ...
                | _ ->
                    previous.PrincipalBalance + previous.FeeBalance
                    - Cent.transferToPortion feeRebateIfSettled
                    + interestPortionL'
                    + Cent.transferToPortion newChargesTotal'
            ...
            let feePortion', feeRebate =
                ...
                then
                    let feeRebate' =
                        ...
                        | Apr.CalculationMethod.UnitedKingdom when feeRebateIfSettled > 0uL<Cent> ->
                            ...
                            |> max (Cent.transferToPortion feeRebateIfSettled)
                            |> min (Cent.transferToPortion feeTotal)
                            |> Cent.portionToTransfer
                        | _ -> feeRebateIfSettled

                    max 0L<Cent> (previous.FeeBalance - Cent.transferToPortion feeRebate'), feeRebate'
                else
                    sign feePortion, 0uL<Cent>
            ...
            let paymentDue', netEffect', (principalPortion': Cent.Signed), (principalBalance': Cent.Signed) =
                if
                    current.PaymentStatus = NotYetDue
                    && feeRebate > 0uL<Cent>
                    && principalBalance < 0L<Cent>
                then
                    paymentDue + Cent.portionToTransfer principalBalance,
            ...
            let carriedCharges, carriedInterestL =
                if netEffect > 0L<Cent> then
                    let nct = Cent.transferToPortion newChargesTotal'

                    if nct > netEffect then
                        Cent.portionToTransfer (nct - netEffect), interestPortionL
                    elif nct + interestPortionL' > netEffect then
                        0uL<Cent>, interestPortionL - (netEffect - nct)
                    else
                        0uL<Cent>, 0L<Cent>
                elif netEffect = 0L<Cent> && interestPortionM < 0m<Cent> then
                    0uL<Cent>, interestPortionL
                else
                    0uL<Cent>, 0L<Cent>
            ...
            let createScheduleItem isSettlement =
                ...
                let (balances: Cent.Signed * Cent.Signed * Cent.PrecisionCent * Cent.Unsigned,
                     interestPortionL',
                     generatedPayment) =
                    if isSettlement then
                        // convert the generated payment placeholder with an actual settlement figure
                        (0L<Cent>, 0L<Cent>, 0m<Cent>, 0uL<Cent>),
                        interestPortionL',
                        ...
                        let feeBalance =
                            previous.FeeBalance - feePortion' - Cent.transferToPortion feeRebate

                        let interestBalance = interestBalanceM |> Interest.ignoreFractionalCents 1u

                        let chargesBalance =
                            previous.ChargesBalance + newChargesTotal - newChargesTotal' + carriedCharges
                ...
                let balanceTotal =
                    principalBal
                    + feeBal
                    + Cent.precisionToPortion p.Basic.InterestConfig.Rounding interestBal
                    + Cent.transferToPortion chargesBal
                ...
                let scheduleItem = {
                    ...
                    FeePortion =
                        if isSettlement then
                            previous.FeeBalance - Cent.transferToPortion feeRebate
                        else
                            feePortion'
                    ...
                    ChargesPortion =
                        if isSettlement then
                            ...
                            newChargesTotal' - carriedCharges
                        |> Cent.transferToPortion
                    FeeRebate = feeRebate
            ...
            let totals'' = {
                totals' with
                    CumulativeScheduledPayments =
                        Cent.transferToPortion totals'.CumulativeScheduledPayments
                        + scheduledPaymentAdjustment
                        |> Cent.portionToTransfer
                    CumulativeGeneratedPayments = totals.CumulativeGeneratedPayments + generatedPayment
                    CumulativeFee =
                        Cent.transferToPortion totals.CumulativeFee + feePortion'
                        |> Cent.portionToTransfer
                    CumulativeInterest = totals'.CumulativeInterest - interestRoundingDifferenceM
                    CumulativeInterestPortions =
                        Cent.transferToPortion totals.CumulativeInterestPortions
                        + scheduleItem.InterestPortion
                        |> Cent.portionToTransfer
            }
        ...
        |> Array.scan
            ...
            (0u<OffsetDay>,
             createInitialScheduleItem
                 p.Basic.StartDate
                 p.Basic.Principal
                 (Cent.transferToPortion feeTotal)
                 initialInterestBalanceM
                 p.Advanced.FeeConfig),
    ...
    let calculateStats (items: Map<uint<OffsetDay>, ScheduleItem>) =
...

        {
            ScheduleItems = items
            FinalStats = {
                RequiredScheduledPaymentCount = requiredScheduledPaymentItems |> Map.count |> uint
                ...
                FinalActualPaymentCount = items' |> Array.sumBy (fun asi -> Map.count asi.ActualPayments |> uint)
                ...
                EffectiveInterestRate =
                    if
                        finalPaymentDay = 0u<OffsetDay>
                        || principalTotal + feeTotal - Cent.transferToPortion feeRebate = 0L<Cent>
                    then
                        0m
                    else
                        decimal interestTotal
                        / decimal (principalTotal + feeTotal - Cent.transferToPortion feeRebate)

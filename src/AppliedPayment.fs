...
module AppliedPayment =
    ...
    type AppliedCharge = {
        ...
        member ac.Html = $"<i>{ac.ChargeType}</i> {ac.Total:N2}"
    ...
    type private SP = {
        spDay: uint<OffsetDay>
        spTotal: Cent.Signed
    }

    type private AP = {
        apDay: uint<OffsetDay>
        apIndex: Index
        apTotal: Cent.Signed
        spp: SP list
    }
    ...
    let applyPayments (parameters: Parameters) actualPayments scheduledPayments =
        ...
        if Map.isEmpty scheduledPayments then
            ...
            let evaluationDay =
                OffsetDay.fromDate parameters.Basic.StartDate parameters.Basic.EvaluationDate
                ...
                |> Array.mapFold
                    (fun aggregateAppliedCharges offsetDay ->
                        ...
                        let confirmedPaymentTotal =
                            actualPayments' |> Map.values |> Seq.sumBy ActualPayment.totalMade
                        // of the actual payments made on the day, sum any that are still pending
                        let pendingPaymentTotal =
                            actualPayments' |> Map.values |> Seq.sumBy ActualPayment.totalPending
                        ...
                        let netEffect, paymentStatus =
                            ...
                            // otherwise, calculate as normal
                                ...
                                | 0uL<Cent>, 0L<Cent> -> 0L<Cent>, NoneScheduled
                                // no payment due, but a refund issued
                                | 0uL<Cent>, cpt when cpt < 0L<Cent> -> cpt, Refunded
                                // no payment due, but a payment made
                                | 0uL<Cent>, cpt -> cpt, ExtraPayment
                                // a payment due on or before the day
                                | spt, cpt when
                                    cpt < Cent.transferToPortion spt
                                    ...
                                    | _ -> Cent.transferToPortion spt, PaymentDue
                                // a payment due on a future day
                                | spt, _ when offsetDay > evaluationDay -> Cent.transferToPortion spt, NotYetDue
                                // a payment due but no payment made
                                | spt, 0L<Cent> when spt > 0uL<Cent> -> 0L<Cent>, MissedPayment
                                // a payment due but the payment made is less than what's due
                                | spt, cpt when cpt < Cent.transferToPortion spt -> cpt, Underpayment
                                // a payment due but the payment made is more than what's due
                                | spt, cpt when cpt > Cent.transferToPortion spt -> cpt, Overpayment
            ...
            // determine which scheduled payments are paid off by which actual payments
            let mapScheduledPayments appliedPayments =
                let scheduledPaymentList =
                    scheduledPayments
                    |> Map.fold
                        (fun acc d sp ->
                            let spt = ScheduledPayment.total sp
                            // filter out cancelled payments
                            if spt > 0uL<Cent> then
                                {
                                    spDay = d
                                    spTotal = ScheduledPayment.total sp |> Cent.transferToPortion
                                }
                                :: acc
                            else
                                acc
                ...
                let applyPayment (spp: SP list) (appliedPaymentTotal: int64<Cent>) =
                    if appliedPaymentTotal >= 0L<Cent> then
                        let folder (apt: Cent.Signed, sppCovered, sppUncovered) sp =
                ...
                |> Map.map (fun d ap -> {
                    ap with
                        ...
                        // //         actp with
                        // //             ScheduledPayments =
                        // //                 paymentMappings |> Map.tryFind (d, i) |> Option.defaultValue Map.empty
                        // //     })
                        PaidBy =
                            paymentMappings
                            |> Map.toSeq
                            |> Seq.choose (fun (k, spp) ->
                                spp |> Map.tryFind d |> Option.map (fun spTotal -> k, spTotal)
                            )
                            |> Map.ofSeq
                })

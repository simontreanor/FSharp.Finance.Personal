...
module AppliedPayment =
    ...
    let applyPayments asOfDay (latePaymentGracePeriod: int<DurationDay>) (latePaymentCharge: Amount voption) (generatedPayment: CustomerPayment voption) actualPayments scheduledPayments =
        ...
        |> Array.map(fun (offsetDay, payments) ->
            ...
            let generatedPayment' = payments |> Array.tryPick(fun p -> p.PaymentDetails |> function GeneratedPayment gp -> Some gp | _ -> None) |> function Some x -> ValueSome x | _ -> ValueNone
            let generatedPaymentTotal = generatedPayment' |> ValueOption.map (GeneratedPayment >> CustomerPaymentDetails.total) |> ValueOption.defaultValue 0L<Cent>

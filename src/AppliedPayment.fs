        ...
        let payments =
            ...
            |> Array.map(fun (offsetDay, payments) ->
                ...
                let confirmedPayments =
                    actualPayments'
                    |> Array.choose(function ActualPayment.Confirmed ap -> Some ap | _ -> None)
                    |> Array.sum

                let pendingPayments =
                    actualPayments'
                    |> Array.choose(function ActualPayment.Pending ap -> Some ap | _ -> None)
                    |> Array.sum
                ...
                let charges =
                    payments
                    |> Array.collect(_.PaymentDetails >> function ActualPayment (ActualPayment.Failed (_, c)) -> c | _ -> [||])
                    |> fun pcc ->

...
module Scheduling =
    ...
    type PaymentStatus =
        ...
        | Generated
        /// no payment is needed because the loan has already been settled
        | NoLongerRequired
        /// a schedule item was generated to show the balances on the as-of date
        | InformationOnly
    ...
    let calculate toleranceOption sp =
        ...
        // note: for simplicity, the principal includes fees
        ...
        // generates a schedule item for a particular day by calculating the interest accruing and apportioning the scheduled payment to interest then the principal
        ...
        // the intention is to use this generator in an iteration by varying the payment value until the final principal balance is zero
    ...
    let mergeScheduledPayments (scheduledPayments: (int<OffsetDay> * ScheduledPayment) array) =
        ...
        |> Array.choose(fun (offsetDay, map) ->
            ...
            // split any rescheduled payments into the latest and previous

            ...
            let newPaymentSchedule =
                ...
                | RegularSchedule _ ->
                    ...
                    | ValueSome s ->
                        ...
                        |> Array.map(fun si -> { PaymentDay = si.Day; PaymentDetails = ScheduledPayment { ScheduledPaymentType = ScheduledPaymentType.Rescheduled si.Payment.Value; Metadata = Map.empty }; OriginalSimpleInterest = 0L<Cent>; ContractualInterest = 0m<Cent> })
                    | ValueNone ->
                        [||]
                | RegularFixedSchedule regularFixedSchedules ->
                    ...
                    |> Array.map(fun rfs ->
                        ...
                        |> Array.map(fun d -> { PaymentDay = OffsetDay.fromDate sp.StartDate d; PaymentDetails = ScheduledPayment { ScheduledPaymentType = ScheduledPaymentType.Rescheduled rfs.PaymentAmount; Metadata = Map.empty }; OriginalSimpleInterest = 0L<Cent>; ContractualInterest = 0m<Cent> })
                    )
                    |> Array.concat

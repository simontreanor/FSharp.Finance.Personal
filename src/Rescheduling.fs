    ...
    let reschedule sp (rp: RescheduleParameters) (actualPayments: CustomerPayment array) =
        voption {
            ...
            let newPaymentSchedule =
                ...
                | RegularSchedule _ ->
                    ...
                    | ValueSome s ->
                        ...
                        |> Array.map(fun si -> { PaymentDay = si.Day; PaymentDetails = ScheduledPayment { ScheduledPaymentType = ScheduledPaymentType.Rescheduled si.Payment.Value; Metadata = Map.empty }; InitialInterest = ValueNone })
                    | ValueNone ->
                        [||]
                | RegularFixedSchedule regularFixedSchedules ->
                    ...
                    |> Array.map(fun rfs ->
                        ...
                        |> Array.map(fun d -> { PaymentDay = OffsetDay.fromDate sp.StartDate d; PaymentDetails = ScheduledPayment { ScheduledPaymentType = ScheduledPaymentType.Rescheduled rfs.PaymentAmount; Metadata = Map.empty }; InitialInterest = ValueNone })
                    )
                    |> Array.concat
            ...
            let oldPaymentSchedule =
                ...
                |> Array.map(fun si -> { PaymentDay = si.OffsetDay; PaymentDetails = ScheduledPayment si.ScheduledPayment; InitialInterest = ValueSome si.InitialInterest })

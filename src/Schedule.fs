    ...
    let generate count direction unitPeriodSchedule =
        let adjustMonthEnd monthEndTrackingDay (dt: DateTime) =
            if dt.Day > 15 && monthEndTrackingDay > 28 then
                DateTime(dt.Year, dt.Month, Math.Min(monthEndTrackingDay, DateTime.DaysInMonth(dt.Year, dt.Month)))
            else dt
        let generate =
            ...
            | SemiMonthly (SemiMonthlyConfig (year, month, TrackingDay day1, TrackingDay day2)) ->
                let startDate = DateTime(year, month, Math.Min(day1, DateTime.DaysInMonth(year, month)))
                ...
                Array.collect(fun c -> [|
                    ...
                    startDate.AddMonths (c + offset) |> fun dt -> DateTime(dt.Year, dt.Month, Math.Min(day2, DateTime.DaysInMonth(dt.Year, dt.Month))) |> adjustMonthEnd monthEndTrackingDay
                |])
                >> Array.take count
            | Monthly (multiple, (MonthlyConfig (year, month, TrackingDay day))) ->
                let startDate = DateTime(year, month, Math.Min(day, DateTime.DaysInMonth(year, month)))

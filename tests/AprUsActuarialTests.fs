...
module AprUsActuarialTests =
    ...
    let calculate1 advanceAmount payment paymentCount intervalSchedule advanceDate =
        ...
        let payments = intervalSchedule |> UnitPeriod.generatePaymentSchedule paymentCount Direction.Forward |> Array.map(fun dt -> { TransferType = Payment; TransferDate = dt; Amount = payment })
    ...
    let ``Example (c)(1)(i): Monthly payments (regular first period)`` () =
        let actual = calculate1 500000L<Cent> 23000L<Cent> 24 (Monthly (1, 1978, 2, 10)) (DateTime(1978, 1, 10))
        let expected = Percent 9.69m
        actual |> should equal expected

    [<Fact>]
    let ``Example (c)(1)(ii): Monthly payments (long first period)`` () =
        let actual = calculate1 600000L<Cent> 20000L<Cent> 36 (Monthly (1, 1978, 4, 1)) (DateTime(1978, 2, 10))
        let expected = Percent 11.82m
        actual |> should equal expected

    [<Fact>]
    let ``Example (c)(1)(iii): Semimonthly payments (short first period)`` () =
        let actual = calculate1 500000L<Cent> 21917L<Cent> 24 (SemiMonthly (1978, 3, 1, 16)) (DateTime(1978, 2, 23))
        let expected = Percent 10.34m
        actual |> should equal expected

    [<Fact>]
    let ``Example (c)(1)(iv): Quarterly payments (long first period)`` () =
        let actual = calculate1 1000000L<Cent> 38500L<Cent> 40 (Monthly (3, 1978, 10, 1)) (DateTime(1978, 5, 23))
    ...
    let calculate2 advanceAmount firstPayment regularPaymentAmount regularPaymentCount intervalSchedule advanceDate =
        ...
        let firstFinanceChargeEarnedDate = consummationDate
        let advances = [| { TransferType = Advance; TransferDate = consummationDate; Amount = advanceAmount } |]
        let payments = intervalSchedule |> generatePaymentSchedule regularPaymentCount Direction.Forward |> Array.map(fun dt -> { TransferType = Payment; TransferDate = dt; Amount = regularPaymentAmount })
    ...
    module ExtraExamples =
        ...
        let ``Example (c)(1)(iv) [modified]: Quarterly payments (shorter first period)`` () =
            let actual = calculate1 1000000L<Cent> 38500L<Cent> 40 (Monthly (3, 1978, 10, 1)) (DateTime(1978, 6, 23))
            let expected = Percent 9.15m
            actual |> should equal expected

        [<Fact>]
        let ``Example (c)(1)(iv) [modified]: Quarterly payments (shorter first period: less than unit-period)`` () =
            let actual = calculate1 1000000L<Cent> 38500L<Cent> 40 (Monthly (3, 1978, 10, 1)) (DateTime(1978, 7, 23))
        ...
        [<Fact>]
        let ``Yearly payments`` () =
            let actual = calculate1 100000L<Cent> 50000L<Cent> 5 (Monthly (12, 2023, 11, 30)) (DateTime(2023, 10, 26))

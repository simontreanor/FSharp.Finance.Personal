...
module AprUnitedKingdomTests =
    ...
    let ``UK-old vs UK. Example (c)(1)(i): Monthly payments (regular first period)`` () =
        let actual = calculateUkOld 500000L<Cent> 23000L<Cent> 24 (Monthly (1, MonthlyConfig (1978, 2, 10<TrackingDay>))) (DateTime(1978, 1, 10)) |> fun apr -> apr / 100m
        let expected = calculateUk 500000L<Cent> 23000L<Cent> 24 (Monthly (1, MonthlyConfig (1978, 2, 10<TrackingDay>))) (DateTime(1978, 1, 10)) |> getAprOr 0m
        actual |> should (equalWithin 0.0001) expected

    [<Fact>]
    let ``UK-old vs UK. Example (c)(1)(ii): Monthly payments (long first period)`` () =
        let actual = calculateUkOld 600000L<Cent> 20000L<Cent> 36 (Monthly (1, MonthlyConfig (1978, 4, 1<TrackingDay>))) (DateTime(1978, 2, 10)) |> fun apr -> apr / 100m
        let expected = calculateUk 600000L<Cent> 20000L<Cent> 36 (Monthly (1, MonthlyConfig (1978, 4, 1<TrackingDay>))) (DateTime(1978, 2, 10)) |> getAprOr 0m
        actual |> should (equalWithin 0.0001) expected

    [<Fact>]
    let ``UK-old vs UK. Example (c)(1)(iii): Semimonthly payments (short first period)`` () =
        let actual = calculateUkOld 500000L<Cent> 21917L<Cent> 24 (SemiMonthly (SemiMonthlyConfig (1978, 3, 1<TrackingDay>, 16<TrackingDay>))) (DateTime(1978, 2, 23)) |> fun apr -> apr / 100m
        let expected = calculateUk 500000L<Cent> 21917L<Cent> 24 (SemiMonthly (SemiMonthlyConfig (1978, 3, 1<TrackingDay>, 16<TrackingDay>))) (DateTime(1978, 2, 23)) |> getAprOr 0m
        actual |> should (equalWithin 0.0001) expected

    [<Fact>]
    let ``UK-old vs UK. Example (c)(1)(iv): Quarterly payments (long first period)`` () =
        let actual = calculateUkOld 1000000L<Cent> 38500L<Cent> 40 (Monthly (3, MonthlyConfig (1978, 10, 1<TrackingDay>))) (DateTime(1978, 5, 23)) |> fun apr -> apr / 100m
        let expected = calculateUk 1000000L<Cent> 38500L<Cent> 40 (Monthly (3, MonthlyConfig (1978, 10, 1<TrackingDay>))) (DateTime(1978, 5, 23)) |> getAprOr 0m
        actual |> should (equalWithin 0.0001) expected

    [<Fact>]
    let ``UK-old vs UK. Example (c)(1)(v): Weekly payments (long first period)`` () =
        let actual = calculateUkOld 50000L<Cent> 1760L<Cent> 30 (Weekly (1, DateTime(1978, 4, 21))) (DateTime(1978, 3, 20)) |> fun apr -> apr / 100m
        let expected = calculateUk 50000L<Cent> 1760L<Cent> 30 (Weekly (1, DateTime(1978, 4, 21))) (DateTime(1978, 3, 20)) |> getAprOr 0m
        actual |> should (equalWithin 0.0001) expected

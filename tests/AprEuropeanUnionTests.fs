...
module AprEuropeanUnionTests =
    ...
    module Quirky =
        ...
        let ``APR calculation 1 payment 0L<Cent>`` () =
            EuropeanUnion.calculateApr (Date(2012, 10, 10)) 500_00L<Cent> [| { TransferType = Payment; TransferDate = Date(2012, 10, 10); Value = 500_00L<Cent> } |]
            |> getAprOr -1m
            |> should (equalWithin 0.001) 0m

        [<Fact>] 
        let ``APR calculation 1 payment`` () =
            EuropeanUnion.calculateApr (Date(2012, 10, 10)) 500_00L<Cent> [| { TransferType = Payment; TransferDate = Date(2012, 10, 15); Value = 510_00L<Cent> } |]
            |> getAprOr 0m
            |> should (equalWithin 0.001) (Percent 324.436m |> Percent.toDecimal)

        [<Fact>] 
        let ``APR calculation 2 payments`` () =
            EuropeanUnion.calculateApr (Date(2012, 10, 10)) 500_00L<Cent> [| { TransferType = Payment; TransferDate = Date(2012, 11, 10); Value = 270_00L<Cent> }; { TransferType = Payment; TransferDate = Date(2012, 12, 10); Value = 270_00L<Cent> } |]
            |> getAprOr 0m
    ...
    let outputHtmlToFile folder startDate paymentCounts firstPaymentDays interestMethod applyInterestCap title description =
        let tableCells firstPaymentDay =
            ...
            |> Array.map(fun paymentCount ->
                ...
                basicSchedule |> BasicSchedule.outputHtmlToFile folder $"""AprEuTest_fp{firstPaymentDay.ToString "00"}_pc{paymentCount}""" $"EU APR test amortisation schedule, first payment day {firstPaymentDay}, payment count {paymentCount}" p.Basic
